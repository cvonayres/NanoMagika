...
void FGameplayTagStack::SetTag(FGameplayTag InTag)
{
	if (!Tag.IsValid())
	{
...
void FGameplayTagStack::AddCount(int32 CountToAdd)
{
	if (!Tag.IsValid())
	{
		FFrame::KismetExecutionMessage(TEXT("An invalid tag was passed to AddStack"), ELogVerbosity::Warning);
	...
	if (StackCount > 0)
	{
			StackCount += CountToAdd;
	}
}

void FGameplayTagStack::RemoveCount(int32 CountToRemove)
{
	if (!Tag.IsValid())
	{
		FFrame::KismetExecutionMessage(TEXT("An invalid tag was passed to AddStack"), ELogVerbosity::Warning);
